...
class ConversationAttachmentServiceTest {
    ...
    @BeforeEach void setup() throws Exception {
        ...
        service=new ConversationAttachmentService(props,provider,new ObjectMapper(),new com.myharness.agent.workspace.WorkspaceFileService(registry,props,provider,new ObjectMapper()));
        ...
        var attachment=new TurnAttachmentDTO("9","hello.txt","application/octet-stream",bytes.length,HexFormat.of().formatHex(MessageDigest.getInstance("SHA-256").digest(bytes)),"hello.txt");
        command.setAttachments(List.of(attachment));
        Files.write(workspace.resolve("hello.txt"),bytes);
        server.createContext("/api/v1/agent/turns/7/attachments",exchange -> {
            ...
            byte[] body;
            if(exchange.getRequestURI().getPath().endsWith("/download")){downloads.incrementAndGet();body=bytes;}
    ...
    @Test void readsWorkspaceFileAndRechecksAuthorization() throws Exception {
        String input=service.prepareInput(command,new AttachmentPreparation()).message();
        assertTrue(input.contains("hello.txt"));
        assertFalse(Files.exists(workspace.resolve(".harness")));
        service.prepareInput(command,new AttachmentPreparation());assertEquals(0,downloads.get());assertEquals(4,manifests.get());
        revoked=true;assertThrows(AgentOperationException.class,() -> service.prepareInput(command,new AttachmentPreparation()));
    }
    @Test void rejectsLegacyFileWithoutWorkspaceAssociationBeforeNetwork() {
        var old=command.getAttachments().getFirst();
        command.setAttachments(List.of(new TurnAttachmentDTO(old.id(),old.fileName(),old.mediaType(),old.sizeBytes(),old.sha256(),null)));
        assertThrows(AgentOperationException.class,() -> service.prepareInput(command,new AttachmentPreparation()));
        assertEquals(0,manifests.get());assertEquals(0,downloads.get());
        assertFalse(Files.exists(workspace.resolve(".harness")));
    }
    @Test void cancellationPreventsNetworkAndPreparation() {
        var preparation=new AttachmentPreparation();preparation.cancel();
        assertThrows(AgentOperationException.class,() -> service.prepareInput(command,preparation));assertEquals(0,downloads.get());
    }
    @Test void unsafeIdentifiersNeverBecomePathsOrUrls() {
        command.setTurnId("../8");assertThrows(AgentOperationException.class,() -> service.prepareInput(command,new AttachmentPreparation()));assertEquals(0,manifests.get());
    }

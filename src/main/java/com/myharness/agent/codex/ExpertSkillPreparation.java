...
public class ExpertSkillPreparation {
    ...
    public synchronized List<CodexSkillInput> prepare(StartTurnCommandDTO turn, AttachmentPreparation cancellation) {
        ...
        if(runtime.getSchemaVersion()!=5 || runtime.getProjectRevision()==null || runtime.getSkills()==null || runtime.getSkills().size()>30
                || (runtime.getSchemaVersion()==5 && (runtime.getMcpServers()==null || runtime.getMcpServers().size()>20))
                || runtime.getRuntimeKey()==null || !runtime.getRuntimeKey().matches("[0-9a-f]{64}"))
            throw new AgentOperationException("EXPERT_CONFIG_INVALID","专家运行配置无效，请同步升级服务端和 Agent 到Skill 按需读取协议 V5");
